...
namespace Complete
{
    public class TankMovement : MonoBehaviourPun, IPunObservable //MonoBehaviourPun
    {
        ...
        private void Update ()
        {
            if (photonView.IsMine == false && PhotonNetwork.IsConnected == false)
                return;
            // Store the value of both input axes.  1或-1
        ...
        void FixPosition()
        {
            if (!photonView.IsMine)
            {
                ...
                if (startPosition != endPosition)
                {
                    ...
                    m_Rigidbody.MovePosition(Vector3.Lerp(startPosition, endPosition, elapsedTime / InterpolationDuration));
                    //Debug.Log("移動捕間執行");
                }
            }
        }
                ...
                if (startRotation != endRotation)
                    {
                        ...
                        this.transform.rotation = Quaternion.Lerp(startRotation, endRotation, elapsedRotationTime / InterpolationDuration);
                        //Debug.Log("旋轉捕間執行");
                    }
            }
        }
        ...
        void IPunObservable.OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            ...
            if (stream.IsWriting)
            {
                ...
                //Debug.Log("位置捕間傳送:" + this.transform.rotation);


                // 旋轉資訊傳送
                stream.SendNext(this.transform.rotation);
                //Debug.Log("旋轉捕間傳送:" + this.transform.rotation);
            }
            else
            {
